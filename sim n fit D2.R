...
# 1. Prepare data for D2

# 1.1 Add ID cohorts

P <- (1/.27222) / c(rep(15,1), rep(15*2,1), rep(15*3,1), rep(15*4,9))
breaks <- cumsum(round(P*n))[1:11]
...
# 1.2 Select rows for each cohort
sup_data_coh <- list()
...
for (j in 1:n_sim){
  ...
  for (i in seq_along(p2)) {
    ...
    row_indices <- c(col_value, col_value + 3) # D2
...
setwd("C:/Users/nuria/OneDrive/Escritorio/Universidad/TFM-Paper/Sim v4/Results D2")

saveRDS(BLCS.fit, paste0("BLCS.fit_D2_",n,".rds"))                                     #!#
...
setwd("C:/Users/nuria/OneDrive/Escritorio/Universidad/TFM-Paper/Sim v4/Results D2")
saveRDS(summaries, paste0("summaries_D2_",n,".rds"))                                   #!#
